...
class Nat(InductiveType):
    ...
    def zero(cls) -> Nat:
        """Construct zero."""
        return cls(EConst(NatDeclaration.NAT_ZERO_DECLARATION.declaration.name, levels=()))

    @classmethod
    def succ(cls, n: Nat | Expr) -> Nat:
        """Construct the successor of a natural number."""
        value = cls.to_expr(n)
        return cls(EApp(EConst(NatDeclaration.NAT_SUCC_DECLARATION.declaration.name, levels=()), value))
    ...
    def add(cls, a: Nat | Expr, b: Nat | Expr) -> Nat:
        ...
        lhs = cls.to_expr(a)
        rhs = cls.to_expr(b)
        return cls(EApp(EApp(EConst(NatDeclaration.NAT_ADD_DECLARATION.declaration.name, levels=()), lhs), rhs))
