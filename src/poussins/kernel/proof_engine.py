...
class ProofEngine:
    ...
    @property
    def definitions(self) -> dict[str, Expr | None]:
        """
        Return unfoldable constant definitions from the environment.
        """
        return {
            name: decl.value
    ...
    def refresh_goal(self, goal: Goal) -> Goal:
        ...
        """
        return goal.with_context(self.global_context | goal.local_context, goal.local_hypothesis_names)
    ...
    def refresh_state(self, state: ProofState) -> ProofState:
        ...
        if not state.goals:
            return state
        return ProofState(goals=tuple(self.refresh_goal(goal) for goal in state.goals), metavars=state.metavars)
    ...
    def create_initial_state(self, statement: Expr) -> ProofState:
        ...
        """
        initial_goal = Goal(statement=statement, context=self.global_context, local_hypothesis_names=frozenset())
    ...
    def change_goal(self, state: ProofState, new_statement: Expr) -> ProofState:
        ...
        if not is_def_eq(new_statement, current_goal.statement, current_goal.context, state.metavars, self.definitions):
            raise KernelValueError(
        ...
        updated_metavars = state.metavars | {
            current_goal.id: MetaVar(statement=new_statement, assignment=current_meta.assignment)
        }
        return ProofState(goals=updated_goals, metavars=updated_metavars)

    def change_hypothesis(self, state: ProofState, hypothesis_name: str, new_type: Expr) -> ProofState:
        """
        ...
        if not current_goal.has_local_hypothesis(hypothesis_name):
            raise KernelValueError(f"change failed: Unknown hypothesis '{hypothesis_name}'.")
        current_type = current_goal.local_context[hypothesis_name]

        if not is_def_eq(new_type, current_type, current_goal.context, state.metavars, self.definitions):
            raise KernelValueError(
                f"change failed: New type for '{hypothesis_name}' is not definitionally equal to the current type."
            )

        updated_local_context = current_goal.local_context | {hypothesis_name: new_type}
        ...
        current_meta = state.metavars[current_goal.id]
        updated_metavars = state.metavars | {
            current_goal.id: MetaVar(statement=current_goal.statement, assignment=current_meta.assignment)
        }
        return ProofState(goals=updated_goals, metavars=updated_metavars)
