...
class ProofManager:
    ...
    def __init__(self, statement: Expr, env: Environment):
        ...
        self.root_metavar_id = (
            initial_state.current_goal.id if initial_state.current_goal else None
        )
    ...
    def refine_goal(self, assignment: Expr, subgoals: list[Goal]):
        """Refine the current goal by providing an assignment."""
        next_state = self.engine.refine_goal(
    ...
    def change_goal(self, new_statement: Expr) -> None:
        ...
        next_state = self.engine.change_goal(
            self.current_state,
            new_statement,
            self.env
        )
    ...
    def change_hypothesis(self, hypothesis_name: str, new_type: Expr) -> None:
        ...
        next_state = self.engine.change_hypothesis(
            self.current_state,
            hypothesis_name,
            new_type,
            self.env
