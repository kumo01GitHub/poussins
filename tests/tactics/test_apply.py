...
def test_apply_closes_goal_with_direct_constructor(default_env: Environment) -> None:
...
def test_apply_raises_when_no_goals_remain(default_env: Environment) -> None:
    manager = ProofManager(EConst("True", ()), default_env)
    apply(manager, EConst("True.intro", ()))
...
def test_apply_raises_when_current_goal_is_missing(default_env: Environment) -> None:
    class DummyManager:
...
def test_apply_enters_product_subgoal_path(default_env: Environment) -> None:
    manager = ProofManager(EConst("True", ()), default_env)
...
def test_apply_wraps_kernel_failure(default_env: Environment) -> None:
    class DummyManager:
        def __init__(self) -> None:
            self.is_closed = False
            self.current_state = SimpleNamespace(
                current_goal=SimpleNamespace(
                    ...
                    context={name: decl.type for name, decl in default_env.items()},
                ),
                metavars={},
            )
